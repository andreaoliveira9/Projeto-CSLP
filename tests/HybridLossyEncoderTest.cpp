    ...
    for (int shift = 1; shift < 4; shift++) {
      for (int i = 0; i < files.size(); i++)
      {
          ...
          auto start_full = chrono::high_resolution_clock::now(); // start timer
            ...
            cout << "Encoding " << files[i] << " with block range " << block_range << "and lossy lever" << shift << endl;
            string output = dir + files[i] + "/" + files[i] + "_lossy_level_" + to_string(shift) + ".bin";

            HybridEncoder encoder(input, 20, block_range, shift);
